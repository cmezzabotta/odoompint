...
class ResConfigSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        for key, value in param_map.items():
            params.set_param(f"mezztt_mp_qr.{key}", value or False)
    ...
    def get_values(self):
        ...
        params = self.env['ir.config_parameter'].sudo()
        res.update({
            'mp_access_token': params.get_param('mezztt_mp_qr.mp_access_token'),
            'mp_public_key': params.get_param('mezztt_mp_qr.mp_public_key'),
            'mp_user_id': params.get_param('mezztt_mp_qr.mp_user_id'),
            'mp_collector_id': params.get_param('mezztt_mp_qr.mp_collector_id'),
            'mp_integrator_id': params.get_param('mezztt_mp_qr.mp_integrator_id'),
            'mp_sponsor_id': params.get_param('mezztt_mp_qr.mp_sponsor_id'),
            'mp_external_store_id': params.get_param('mezztt_mp_qr.mp_external_store_id'),
            'mp_external_pos_id': params.get_param('mezztt_mp_qr.mp_external_pos_id'),
            'mp_pos_id': params.get_param('mezztt_mp_qr.mp_pos_id'),
            'mp_qr_mode': params.get_param('mezztt_mp_qr.mp_qr_mode', default='dynamic'),
            'mp_notification_url': params.get_param('mezztt_mp_qr.mp_notification_url'),
        })
        return res

    def action_test_connection(self):
        self.ensure_one()
        client = self.env['mezztt_mp_qr.mercadopago_client']
        try:

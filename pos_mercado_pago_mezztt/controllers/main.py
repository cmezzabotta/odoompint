...
class MpMezzttController(http.Controller):
    ...
    def _get_credentials() -> Dict[str, Any]:
        ...
        return {
            ...
            "collector_id": icp.get_param(PARAM_COLLECTOR_ID, default=""),
            "pos_id": icp.get_param(PARAM_POS_ID, default=""),
            "store_id": icp.get_param(PARAM_STORE_ID, default=""),
        }
    ...
    def mp_create(self, **_payload):
        ...
        collector_id = creds.get("collector_id")
        pos_id = creds.get("pos_id")
        if not collector_id or not pos_id:
            return {
                "error": "Configura Collector ID y POS ID en Ajustes de POS para generar el QR."
            }
        ...
        mp_url = (
            f"{_base_url(creds['sandbox'])}/instore/orders/qr/seller/collectors/"
            f"{collector_id}/pos/{pos_id}/qrs"
        )
        ...
        if store_id:
            # TODO: reemplazar sponsor_id por la clave adecuada para enviar store_id a MP.
            body.setdefault("sponsor_id", store_id)
